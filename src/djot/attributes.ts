...
handlers[State.FAIL] = function(_parser : AttributeParser, _pos : number) {
  return State.FAIL;
}

handlers[State.DONE] = function(_parser : AttributeParser, _pos : number) {
  return State.DONE;
}
...
handlers[State.SCANNING_ESCAPED] = function(_parser : AttributeParser, _pos : number) {
  return State.SCANNING_QUOTED_VALUE;
}

handlers[State.SCANNING_ESCAPED_IN_CONTINUATION] = function(_parser : AttributeParser, _pos : number) {
